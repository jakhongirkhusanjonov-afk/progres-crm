...
export default function GroupProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    const filtered = allStudents.filter((s) => {
      ...
      const isInGroup = group?.groupStudents.some((gs) => gs.student.id === s.id)
  ...
  const handleRemoveStudent = async (studentId: string) => {
    try {
      const token = localStorage.getItem('token')
      const response = await fetch(`/api/groups/${id}/students?studentId=${studentId}`, {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  const formatScheduleDays = (days?: string) => {
    if (!days) return '-'
    return days.split(',').map((d) => DAYS_OF_WEEK[d] || d).join(', ')
  }
  ...
  const studentsColumns: ColumnsType<GroupStudent> = [
    ...
    },
    {
      ...
      key: 'fullName',
      render: (_, record) => (
        <a onClick={() => router.push(`/dashboard/students/${record.student.id}`)}>
    ...
    },
    {
      ...
      key: 'enrollDate',
      render: (_, record) => new Date(record.enrollDate).toLocaleDateString('uz-UZ'),
    },
    {
          ...
          return (
            <span>
              ...
              {record.discountReason && (
                <div className="text-xs text-gray-500">{record.discountReason}</div>
              )}
            </span>
          )
        }
          ...
          <div className="flex justify-between h-16 items-center">
            <div className="flex items-center gap-8">
              ...
              <nav className="hidden md:flex gap-4">
                ...
                <a href="/dashboard/groups" className="text-indigo-600 font-medium">
                  Guruhlar
                </a>
                <a href="/dashboard/courses" className="text-gray-600 hover:text-gray-900">
                  Kurslar
                </a>
              </nav>
            </div>
                  ...
                  <div>
                    ...
                    <div className="mb-4 flex justify-between items-center">
                      ...
                      <Button
                        type="primary"
                        icon={<PlusOutlined />}
                        onClick={() => router.push(`/dashboard/attendance/mark?groupId=${id}`)}
                      >
                        Davomat belgilash
                      </Button>
                    </div>
                    ...
                    {loadingAttendance ? (
                      ...
                      <>
                        ...
                        <Row gutter={[16, 16]} className="mb-4">
                          ...
                          <Col xs={12} sm={6}>
                            <Card className="text-center bg-green-50" size="small">
                              <Statistic
                                title="Kelgan"
                                value={(attendanceStats?.present || 0) + (attendanceStats?.late || 0)}
                          ...
                          <Col xs={12} sm={6}>
                            <Card className="text-center bg-red-50" size="small">
                              <Statistic
                                title="Kelmagan"
                        ...
                        {studentRankings && (studentRankings.best || studentRankings.worst) && (
                          <Row gutter={[16, 16]} className="mb-4">
                                ...
                                <Card size="small" className="border-green-200 bg-green-50">
                                  <div className="flex items-center gap-3">
                                    ...
                                    <div className="flex-1">
                                      <div className="text-xs text-gray-500">Eng yaxshi davomat</div>
                                      <a
                                        onClick={() => router.push(`/dashboard/students/${studentRankings.best!.student.id}`)}
                                        className="font-medium text-green-700 hover:text-green-800"
                                      >
                                        {studentRankings.best.student.lastName} {studentRankings.best.student.firstName}
                                      </a>
                                    </div>
                                    <div className="text-right">
                                      <div className="text-2xl font-bold text-green-600">
                                        {studentRankings.best.stats.attendanceRate}%
                                      </div>
                                      <div className="text-xs text-gray-500">
                                        {studentRankings.best.stats.present + studentRankings.best.stats.late}/{studentRankings.best.stats.total}
                                      </div>
                                    </div>
                                  </div>
                                </Card>
                            ...
                            {studentRankings.worst && studentRankings.best?.student.id !== studentRankings.worst?.student.id && (
                              <Col xs={24} sm={12}>
                                <Card size="small" className="border-red-200 bg-red-50">
                                  <div className="flex items-center gap-3">
                                    ...
                                    <div className="flex-1">
                                      <div className="text-xs text-gray-500">Eng past davomat</div>
                                      <a
                                        onClick={() => router.push(`/dashboard/students/${studentRankings.worst!.student.id}`)}
                                        className="font-medium text-red-700 hover:text-red-800"
                                      >
                                        {studentRankings.worst.student.lastName} {studentRankings.worst.student.firstName}
                                      </a>
                                    </div>
                                    <div className="text-right">
                                      <div className="text-2xl font-bold text-red-600">
                                        {studentRankings.worst.stats.attendanceRate}%
                                      </div>
                                      <div className="text-xs text-gray-500">
                                        {studentRankings.worst.stats.present + studentRankings.worst.stats.late}/{studentRankings.worst.stats.total}
                                      </div>
                                    </div>
                                  </div>
                                </Card>
                        ...
                        {attendanceSessions.length > 0 ? (
                          <Table
                            ...
                            rowKey={(record) => `${record.groupId}-${record.date}`}
              ...
              notFoundContent={
                studentSearchText ? (
                  searchStudents.length === 0 ? (
                    <Empty description="Talaba topilmadi" image={Empty.PRESENTED_IMAGE_SIMPLE} />
                  ) : null
                ) : (
                  <div className="p-2 text-gray-500">Qidirish uchun yozing...</div>
                )
              }
            >
