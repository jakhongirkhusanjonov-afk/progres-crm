...
export default function TeacherProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchAttendanceStats = async () => {
    ...
    try {
      const token = localStorage.getItem('token')
      const response = await fetch(`/api/attendance/teacher/${id}?month=${selectedMonth}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
  ...
  const fetchSalary = async () => {
    ...
    try {
      const token = localStorage.getItem('token')
      const response = await fetch(`/api/salary/teacher/${id}?month=${selectedMonth}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
  ...
  const openLoginModal = () => {
    ...
    if (teacher) {
      const defaultUsername = `${teacher.lastName.toLowerCase()}.${teacher.firstName.toLowerCase()}`.replace(/[^a-z0-9._]/g, '')
  ...
  // Login yaratish
  const handleCreateLogin = async (values: { username: string; password: string }) => {
  ...
  const canCreateLogin = currentUser && ['SUPER_ADMIN', 'ADMIN'].includes(currentUser.role)
  ...
  const groupsColumns: ColumnsType<Group> = [
    ...
    },
    {
      ...
      render: (_, record) => {
        ...
        const c = config[record.status] || { color: 'default', text: record.status }
    ...
    },
    {
      ...
      render: (_, record) => (
        <Button
          type="link"
          size="small"
          onClick={() => router.push(`/dashboard/attendance/mark?groupId=${record.id}`)}
        >
          Davomat belgilash
        </Button>
      ),
  ...
  const attendanceColumns: ColumnsType<GroupStats> = [
    ...
    },
    {
      ...
      render: (_, record) => <Tag color="purple">{record.teacherPercentage}%</Tag>,
    },
  ]
          ...
          <div className="flex justify-between h-16 items-center">
            <div className="flex items-center gap-8">
              ...
              <nav className="hidden md:flex gap-4">
                ...
                <a href="/dashboard/teachers" className="text-indigo-600 font-medium">
                  O'qituvchilar
                </a>
                <a href="/dashboard/groups" className="text-gray-600 hover:text-gray-900">
                  Guruhlar
                </a>
                <a href="/dashboard/attendance" className="text-gray-600 hover:text-gray-900">
                  Davomat
                </a>
              </nav>
            </div>
                ...
                children: (
                  <Descriptions bordered column={{ xs: 1, sm: 2 }}>
                    ...
                    <Descriptions.Item label="Telefon">
                      <a href={`tel:${teacher.phone}`} className="text-blue-600">
                        {teacher.phone}
                      </a>
                    </Descriptions.Item>
                    ...
                    <Descriptions.Item label="Fanlar" span={2}>
                      {teacher.teacherCourses.length > 0 ? (
                        ...
                        <span className="text-gray-400">Hech qanday fan biriktirilmagan</span>
                      )}
                    </Descriptions.Item>
                  </Descriptions>
                ),
                  ...
                  <div>
                    ...
                    {attendanceLoading ? (
                      ...
                      <>
                        ...
                        {Object.keys(attendanceData.recentLessons).length > 0 && (
                          <>
                            <Divider>O'tgan darslar ro'yxati</Divider>
                            <Table
                              size="small"
                              dataSource={Object.entries(attendanceData.recentLessons).flatMap(
                                ([date, lessons]) =>
                                  lessons.map((lesson, idx) => ({
                                    ...
                                    rate:
                                      lesson.present + lesson.absent > 0
                                        ? Math.round((lesson.present / (lesson.present + lesson.absent)) * 100)
                                        : 0,
                                  }))
                              )}
                              ...
                              columns={[
                                ...
                                },
                                {
                                  ...
                                  key: 'group',
                                  render: (_, record: any) => (
                                    <a onClick={() => router.push(`/dashboard/groups/${record.groupId}`)}>
                                      {record.groupName}
                                    </a>
                                  ),
                                },
                                ...
                                },
                                {
                                  ...
                                  render: (_, record: any) => (
                                    <Tag
                                      color={
                                        record.rate >= 80 ? 'green' : record.rate >= 50 ? 'orange' : 'red'
                                      }
                                    >
                                      {record.rate}%
                                    </Tag>
                        ...
                        <div className="mt-4 p-4 bg-gray-50 rounded-lg">
                          ...
                          <p className="text-gray-600 text-sm">
                            ...
                            <code className="bg-gray-200 px-2 py-1 rounded">
                              Talabalar individual narxlari yig'indisi × O'qituvchi foizi × Davomat koeffitsienti
                            </code>
                          </p>
                          <p className="text-gray-500 text-xs mt-2">
                            * Davomat koeffitsienti = O'tilgan darslar / Jami darslar
                          </p>
                        </div>
                      </>
                    ) : (
                      ...
                      <>
                          ...
                          <Col xs={12} sm={6}>
                            <Card size="small" className="text-center bg-red-50">
                              <Statistic
                                ...
                                formatter={(val) => formatPrice(Number(val))}
                                valueStyle={{ color: salaryData.summary.debt > 0 ? '#ff4d4f' : '#52c41a', fontSize: '14px' }}
                                prefix={salaryData.summary.debt > 0 ? <ExclamationCircleOutlined /> : <CheckCircleOutlined />}
                              />
                            </Card>
                          </Col>
                        </Row>
                            ...
                            columns={[
                              {
                                ...
                                render: (_, record) => (
                                  <div>
                                    <div className="font-medium">{record.group.name}</div>
                                    <div className="text-xs text-gray-500">{record.group.course}</div>
                                  </div>
                                ),
                              },
                              {
                                ...
                                key: 'students',
                                width: 100,
                                render: (_, record) => `${record.studentCount} ta`,
                              },
                              {
                                title: "To'lovlar",
                                key: 'payments',
                              ...
                              },
                              {
                                ...
                                width: 80,
                                render: (_, record) => (
                                  <Tag icon={<CalendarOutlined />}>{record.lessonsCount}</Tag>
                                ),
                              },
                              {
                                title: 'Koeffitsient',
                                ...
                                render: (_, record) => (
                                  <Progress
                                    percent={Math.round(record.attendanceCoefficient * 100)}
                                    size="small"
                                    format={(percent) => `${(record.attendanceCoefficient).toFixed(2)}`}
                                  />
                                ),
                              },
                              {
                        ...
                        <div className="mt-4 text-right">
                          <Button
                            type="primary"
                            icon={<RightOutlined />}
                            onClick={() => router.push(`/dashboard/salary/teacher/${id}?month=${selectedMonth}`)}
                          >
                            Batafsil ko'rish
                          </Button>
                        </div>
        ...
        >
          <Form.Item
            ...
            rules={[
              ...
              { pattern: /^[a-z0-9_\.]+$/, message: 'Faqat kichik harflar, raqamlar, _ va .' },
            ]}
          >
            <Input
          ...
          >
            <Input.Group compact>
              <Input
                ...
                onChange={(e) => loginForm.setFieldsValue({ password: e.target.value })}
              />
              <Button
                type="primary"
        ...
        {createdCredentials && (
          <div className="bg-gray-50 p-4 rounded-lg border-2 border-dashed border-gray-300 mb-4">
            <div className="flex justify-between items-center mb-2">
              ...
              <Typography.Text strong copyable>{createdCredentials.username}</Typography.Text>
            </div>
            <div className="flex justify-between items-center">
              <Typography.Text type="secondary">Parol:</Typography.Text>
              <Typography.Text strong copyable>{createdCredentials.password}</Typography.Text>
            </div>
          </div>
        )}
